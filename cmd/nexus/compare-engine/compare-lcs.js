...
function buildTree($el, $) {
  ...
  $el.children().each((i, c) => {
    if (i < 80) { const ch = buildTree($(c), $); if (ch) node.ch.push(ch); }
  });
  return node;
}
...
(async () => {
  ...
  // Score: missing nodes weighted higher than class changes
  ...
  console.log(`NX-only: ${r.un1.length} | GD-only: ${r.un2.length} | Class diffs: ${r.classChanges.length}\n`);

  if (r.un1.length) {
    console.log(`MISSING from GD (elements in NX not in GD):`);
    for (const n of r.un1.slice(0, 8))
      console.log(`  <${n.t}> "${n.x}" [${(n.c||'').substring(0,60)}]`);
  }
  
  if (r.un2.length) {
    console.log(`\nEXTRA in GD (elements in GD not in NX):`);
    for (const n of r.un2.slice(0, 8))
      console.log(`  <${n.t}> "${n.x}" [${(n.c||'').substring(0,60)}]`);
  }
  
  if (r.classChanges.length) {
    console.log(`\nCLASS CHANGES (same element, different CSS classes):`);
    for (const d of r.classChanges.slice(0, 10)) {
      console.log(`  <${d.t}> "${d.x}"`);
